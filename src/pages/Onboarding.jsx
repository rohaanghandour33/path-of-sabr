...
const QUESTIONS = [
  ...
  },
  {
    ...
    placeholder:
      "e.g. my friends don't understand why I'm changing, my family isn't Muslim, I'm dealing with something I'm ashamed of...",
  ...
  },
  {
    ...
    placeholder:
      'Describe it freely — how would you feel, what would be different, what would your relationship with Allah look like...',
  },
  {
    step: 6,
    id: 'q7',
...
function ProgressBar({ step }) {
  ...
  return (
    <div className="sticky top-0 z-10 px-5 pt-12 pb-4" style={{ background: '#051a10' }}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-medium text-white/40">Question {step + 1} of 8</span>
        <span className="text-xs font-semibold text-[#1D9E75]">{pct}%</span>
      </div>
      ...
      {options.map((opt) => (
        <button
          ...
          className={`w-full text-left px-5 py-4 rounded-2xl border transition-colors text-sm leading-snug ${
            value === opt
              ? 'bg-[#1D9E75]/15 border-[#1D9E75]/60 text-white'
              : 'bg-white/5 border-white/10 text-white/70 active:border-white/20'
          }`}
        >
          {opt}
        </button>
        ...
        >
          <div
            className={`mt-0.5 w-5 h-5 rounded flex-shrink-0 flex items-center justify-center border transition-colors ${
              value.includes(opt) ? 'bg-[#1D9E75] border-[#1D9E75]' : 'border-white/25'
            }`}
          >
            {value.includes(opt) && (
...
export default function Onboarding() {
  ...
  const canContinue = () => {
    ...
    if (q.type === 'single') {
      if (!val) return false;
      if (q.subQuestion && val === q.subQuestion.condition) {
        return (responses[q.subQuestion.id] || []).length > 0;
      }
      return true;
    }
  ...
  if (step === 8) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
        <div className="w-16 h-16 rounded-full bg-[#1D9E75]/20 flex items-center justify-center mb-6">
  ...
  const q2onChange = (v) =>
    setResponses((r) => ({ ...r, q2: v, q2b: v === 'No' ? [] : r.q2b }));

  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#051a10' }}>
        ...
        {q.type === 'text' && (
          <div>
            <textarea
              ...
              rows={5}
              className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-4 text-white placeholder-white/25 text-sm resize-none focus:outline-none focus:border-[#1D9E75]/50 transition-colors"
            />
            {q.footnote && (
            ...
            {q.optional && (
              <p className="mt-2 text-xs text-white/20">Optional — tap Continue to skip</p>
            )}
          </div>
        )}
      </div>
      ...
      <div
        className="fixed bottom-0 left-0 right-0 px-5 pb-10 pt-6"
        style={{ background: 'linear-gradient(to top, #051a10 65%, transparent)' }}
      >
        <button
          onClick={handleNext}
